...
export function ForecastSection({ books, className }: ForecastSectionProps) {
    ...
    books.length > 0
      ? books.reduce((prev, curr) => {
          const prevDays = Math.ceil(
            (prev.totalPages - prev.currentPage) / prev.pagesPerDay
          );
          const currDays = Math.ceil(
            (curr.totalPages - curr.currentPage) / curr.pagesPerDay
          );
  ...
  const soonestDays = soonestBook
    ? Math.ceil(
        (soonestBook.totalPages - soonestBook.currentPage) /
          soonestBook.pagesPerDay
      )
    : 0;
    ...
    >
      ...
      <div className="flex items-start justify-between mb-2">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl  text-violet-500 flex items-center justify-center">
        ...
        <Popover>
          ...
          <PopoverContent side="left" className="max-w-xs">
            <p className="text-xs">
              <strong>The Forecast</strong> answers the question: &quot;If I
              keep reading at my current pace, exactly when will I finish each
              book?&quot; Predictions are based on your selected time range
              velocity average.
            </p>
          </PopoverContent>
        </Popover>
      ...
      {soonestBook && soonestDays > 0 && (
        <div className="flex items-center gap-2 mt-4 mb-6 p-3 rounded-xl bg-violet-500/5 border border-violet-500/20">
          ...
          <p className="text-sm text-foreground">
            ...
            <span className="font-semibold">{soonestBook.title}</span> in{" "}
